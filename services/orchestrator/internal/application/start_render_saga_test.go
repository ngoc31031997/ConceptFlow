...
func TestStartRenderSagaUseCase_Execute(t *testing.T) {
	...
	if project.Status != domain.StatusValidatingScript {
		t.Fatalf("expected saved project status validating_script, got %s", project.Status)
	}
	...
	step, err := repo.GetStep(context.Background(), out.SagaID, domain.StepValidateScript)
	if err != nil {
		t.Fatalf("expected validate_script saga step to exist: %v", err)
	}
	if step.Status != domain.SagaStepInProgress {
		t.Fatalf("expected validate_script step in_progress, got %s", step.Status)
	}
	...
	if last.routingKey != "rendering" {
		t.Fatalf("expected routing key rendering, got %s", last.routingKey)
	}
