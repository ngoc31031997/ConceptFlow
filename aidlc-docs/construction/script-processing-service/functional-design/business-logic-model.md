...
3. Với mỗi heading tìm thấy, theo thứ tự xuất hiện:
   ...
   b. Thu thập nội dung từ sau heading này tới heading tiếp theo (hoặc hết file):
      ...
      - Code fence (```` ``` ````...```` ``` ````) → `code_snippet`; nếu gặp code fence thứ 2 trong cùng scene → raise `ScriptSyntaxError` (Rule 5).
   ...
   d. Tạo `Scene(scene_index=N-1, narration_text, illustration_hint, code_snippet)`.
...
flowchart TD
    ...
    H -- No --> I[Tạo Scene scene_index, narration_text, illustration_hint?, code_snippet?]
    I --> D
