...
Chưa làm: FR113 (prompt còn lại của web-gui về server), FR115 (số đo check). Chưa kiểm chứng: lượt `generate` gọi LLM thật qua authoring-service, đường 409 khi xoá lúc đang render (chỉ có test đơn vị), `migrate-authoring-data.sh` với dữ liệu thật (DB hiện chỉ có `llm_usage`).
