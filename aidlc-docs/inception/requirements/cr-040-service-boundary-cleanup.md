...
- **FR116** thẻ tiến độ dùng chung (chưa rebuild/restart, chưa chạy thật với LLM):
  - web-gui: `OperationProgressCard`, `useOperationProgress`, `useOperationRun`; bước 1a/1b/1c dùng thẻ chung (lô của 1c và vòng Repair có phần trăm); `PublishForm`, `ShortScriptAssistant`, chủ đề thumbnail có thẻ "AI đang suy luận/viết · n ký tự · thời gian" và giữ lại số đo + lỗi phân loại khi thất bại; tải ảnh lên theo byte (XHR); xoá project hiện "k/N service" rồi mới bỏ dòng/chuyển trang.
  - `authoring-service`: registry `Operations` trong bộ nhớ + `GET /v1/operations/{id}`; `llm-service` thêm `stream:true` cho hai endpoint suggest để đẩy sự kiện `progress`.
  - orchestrator: `GET /v1/operations/delete:<project_id>` (done/total = số service đã purge); gateway định tuyến theo tiền tố `delete:`.
  - **Lệch so với FR116.3:** id do GUI tự sinh và gửi qua header `X-Operation-Id` (poll được ngay khi request còn mở) thay vì server trả `operation_id`, để không đổi hình dạng response công khai. Kiểm tra biên dịch/Repair dùng tiếp `authoring/{step}/progress` (đã có `repair_round/repair_max`), chưa gộp vào `/v1/operations`. `elapsed_ms` của thao tác xoá là 0 (GUI không hiện đồng hồ cho thẻ này).

- **FR115** số đo lượt check (chưa rebuild/restart): `rendering` ghi log mỗi lượt check (`wait`, `run`, `renders_running`, `timed_out`) và phơi `GET /metrics/checks` với `checks_total`, `checks_timed_out`, `checks_overlapping_render`, `renders_running`, p95/max thời gian chờ, p95 thời gian chạy (500 lượt gần nhất, trong bộ nhớ). "Số render đang chạy" là số lệnh rendering đang xử lý (consumer prefetch=1 nên là 0 hoặc 1), lấy mẫu lúc check bắt đầu chạy. `timed_out` = tsc quá `TSC_TIMEOUT_SECONDS` hoặc bên gọi (llm-service) bỏ chờ; timeout của Manim dry run nằm trong diagnostics nên chưa đếm. FR115.3 vẫn là điều kiện kích hoạt CR sau: đọc p95 wait, nếu > 60s thì tách.

- **FR113.1/113.2** prompt về server (đã rebuild + chạy thật qua gateway): `scriptPrompts.ts` và `scriptTemplates.ts` bị xoá khỏi web-gui.
  - authoring-service thêm bốn vai trò thư viện `manim_adjust`, `remotion_adjust`, `short_script`, `thumbnail_design` (seed nhúng từ đúng văn bản TypeScript cũ), `POST /v1/prompt-renders` (nhận giá trị nháp của GUI: chủ đề, script dán vào, kiểu phụ đề, format/voice) và `GET /v1/script-templates` (script mẫu + snippet hook/end screen). Thay chuỗi một lượt (không quét lại chữ Creator dán vào, nên `$&` hay `{{topic}}` trong script không bị hiểu nhầm).
  - Golden test: `testdata/prompt_golden.json` sinh từ TypeScript trước khi xoá (adjust Manim/Remotion, short script, thumbnail × vi/en × input rỗng/có; vùng phụ đề × 2 ngôn ngữ × 4 mode × 4 cỡ × 2 vị trí); test Go so từng byte, kể cả qua seed thật. Beat sheet và channel identity đã có golden từ FR111.
  - Sửa lệch cũ: vai trò Remotion dùng luật ngôn ngữ riêng (`narrations`, không phải `self.narrate`); trước đây chỉ trang wizard làm đúng, đường "Chạy bằng AI" dùng luật của Manim.
  - web-gui: `renderPrompt`/`useRenderedPrompt` (debounce; nút Copy tắt khi văn bản đang cũ hoặc chưa tải), các trang Story/Visual Director/Engineer và ScriptAssistant/ShortScriptAssistant/ThumbnailUpload gọi server thay vì ghép chuỗi.

- **FR113.3** (hướng A, Creator chốt): `scriptValidation.ts` bỏ đúng các kiểm tra rendering đã bắt (thiếu class Scene — `script_locator`; thiếu `narrations` hoặc rỗng — `dry_run`; thiếu `<Composition id="creator">` — `script_locator`) và bỏ kiểm tra chuẩn cũ `# NARRATION`/`self.wait(AUTO)` (chuẩn đã gỡ khỏi hệ thống). Giữ những gì server không báo trước lúc render: dòng markdown ``` sót lại, `calculateMetadata`, `<Segments>`, import `registerRoot`, cân ngoặc, và ước lượng thời lượng lời thoại. Các kiểm tra còn giữ vẫn chặn nút Tiếp tục như cũ.

Toàn bộ FR trong CR-040 đã làm. `migrate-authoring-data.sh` không cần chạy trên dữ liệu thật (Creator xác nhận).
