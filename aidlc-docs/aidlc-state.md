...
| CR-038 | Thư viện minh hoạ Lottie cho engine Remotion (`LottieClip`, catalog có cổng giấy phép, công cụ duyệt clip) + bộ avatar mèo mướp `cat.*` (10 biểu cảm/hành động) | P2 | **Hoàn tất (2026-09-26): Creator xác nhận giấy phép, 10 clip `cat.*` đã `approved`** | Lottie thay vì Rive vì chỉ cần phát animation, `@remotion/lottie` chính thức và xác định theo frame. LLM chỉ CHỌN clip theo id, không sinh/sửa Lottie. Chỉ Remotion Engineer nhận catalog (Visual Director giữ trung lập engine). Clip chỉ vào prompt khi `approved` và có `license_checked`; bộ `cat.*` được duyệt 2026-09-26 (Creator xác nhận "Free to use under the Lottie Simple License" trên trang clip gốc). Nhánh: `feature/cr-038-lottie-illustration-library` |
| CR-042 | Cảm giác phim thay vì slide: `narrate` chạy animation trong lúc đọc thoại, bỏ thẻ hook/recap mặc định, quy tắc đạo diễn 12–18 | P1 | **GĐ1–3 đã code, chờ chạy video thật** | Drift tắt mặc định; `recap()` đổi hành vi, bản cũ `recap_card()`/`hook_card()`. SFX/nhạc theo cảm xúc (GĐ4) ở backlog. |
...
- **Saga hiện tại** (sau CR-007/020/021/023/024/029):
  ...
  (`qc_video` tắt khỏi luồng chính từ CR-029, code còn giữ)
