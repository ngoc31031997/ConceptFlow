...
## Step 6b: Generate CI/CD Integration Instructions (ALWAYS)

Create `aidlc-docs/construction/build-and-test/ci-cd-integration-instructions.md` describing how to wire the project's build/test into a CI/CD pipeline, including static code quality (SonarQube) and security scanning (OWASP). Detect the CI platform from the repo if brownfield (e.g., existing `.github/workflows/`, `.gitlab-ci.yml`, `Jenkinsfile`, `azure-pipelines.yml`); if none exists, ask the user which platform to target (GitHub Actions, GitLab CI, Jenkins, Azure DevOps, or other) rather than assuming.
...
## 1. CI/CD Platform
- **Platform**: [Detected or user-selected: GitHub Actions / GitLab CI / Jenkins / Azure DevOps / other]
- **Pipeline config file location**: [e.g., `.github/workflows/ci.yml`]

## 2. SonarQube Integration
**Purpose**: Static code analysis for code quality, maintainability, bugs, code smells, and security hotspots.

- **Prerequisites**:
  - SonarQube server URL (self-hosted or SonarCloud) and auth token stored as a CI secret (never hardcoded)
  - Language-appropriate scanner (`sonar-scanner`, `sonar-maven-plugin`, `sonar-dotnet`, etc.)
- **Configuration file**: `sonar-project.properties` (or build-tool-native config) with:
  - `sonar.projectKey`, `sonar.sources`, `sonar.tests`, `sonar.exclusions`
  - Coverage report path (link unit test coverage report generated in Step 3 into Sonar)
- **Pipeline step** (example, adapt to detected platform):
  \`\`\`bash
  sonar-scanner \
...
- **OWASP Dependency-Check** (dependency/SCA scanning — always include):
  ...
  - Pipeline step (example):
    \`\`\`bash
    dependency-check --project "[project-name]" --scan . --format HTML --format JSON --out reports/dependency-check
    \`\`\`
  - Fail the build on CVSS score above the project's configured threshold (e.g., >= 7 High/Critical)
- **OWASP ZAP** (DAST — include if the unit exposes a running web/API service):
  - Baseline or full scan against a deployed test/staging instance
...
- List all required CI secrets (e.g., `SONAR_TOKEN`, `SONAR_HOST_URL`, registry credentials, deploy keys)
- Never commit secrets; reference the CI platform's secret store

## 5. Pipeline Trigger Rules
- **On Pull Request**: Build, unit test, SonarQube analysis, OWASP Dependency-Check (fast feedback, no deploy)
- **On Merge to Main**: Full pipeline including integration tests, OWASP ZAP (if applicable), package, deploy to target environment

## 6. Local Reproduction
- Commands to run SonarQube and OWASP Dependency-Check locally before pushing, so failures are caught before CI
```
...
## CI/CD & Quality Gates
- **CI/CD Platform**: [Detected/selected platform]
- **SonarQube Quality Gate**: [Pass/Fail/Not yet run]
- **OWASP Dependency-Check**: [Pass/Fail/Not yet run] — [X] vulnerabilities found ([X] High/Critical)
- **OWASP ZAP (if applicable)**: [Pass/Fail/N/A]
