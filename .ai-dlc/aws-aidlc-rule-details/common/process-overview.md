...
• **OPERATIONS PHASE**: Deployment strategy, observability, resilience, and production-readiness (CONDITIONAL)

## The Adaptive Workflow:
• **Workspace Detection** (always) → **Reverse Engineering** (brownfield only) → **Requirements Analysis** (always, adaptive depth) → **Conditional Phases** (as needed) → **Workflow Planning** (always) → **High-Level Design** (conditional) → **Code Generation** (always, per-unit, preceded by conditional Low-Level Design) → **Build and Test** (always)
...
• **All other stages are conditional**: Reverse Engineering, User Stories, High-Level Design, Application Design, Units Generation, per-unit design stages (Low-Level Design, Functional Design, NFR Requirements, NFR Design, Infrastructure Design)
...
- High-Level Design (HLD): System-wide macro architecture — system context, major component/service map, technology direction, integration boundaries (CONDITIONAL)
- Application Design: Detailed component identification and service layer design, refining HLD (CONDITIONAL)
- Units Generation: Decompose into units of work (CONDITIONAL)

**🟢 CONSTRUCTION PHASE** - Design, Implementation, Build and Test
- Low-Level Design (LLD): Unit-level module/class structure, interface contracts, and sequence flows (CONDITIONAL, per-unit)
...
**🟡 OPERATIONS PHASE** - Deployment, Observability, Resilience
- Operations: Deployment strategy, autoscaling verification against NFRs, observability (metrics/tracing/logging/SLOs), incident response, disaster recovery, capacity planning (CONDITIONAL)
