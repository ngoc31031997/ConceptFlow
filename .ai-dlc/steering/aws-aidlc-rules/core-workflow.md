...
- `.ai-dlc/aidlc-rules/aws-aidlc-rule-details/` (typical with AI-assisted setup)
- `.ai-dlc-rule-details/` (typical with Cursor, Cline, Claude Code, GitHub Copilot, OpenAI Codex)
- `.ai-dlc/aws-aidlc-rule-details/` (typical with Kiro IDE and CLI)
...
- Load `common/architecture-decision-records.md` for ADR creation rules
...
**Purpose**: System-wide macro architecture — defines the overall shape of the solution before any component-level detail is worked out.

**Execute IF**:
- New system, major feature, or architectural change is being planned
- Multiple components/services will need to interact
- Technology stack direction or system boundaries are not yet decided
- Stakeholders need a shared, approvable view of the target architecture before detailed design begins

**Skip IF**:
- Change stays entirely within a single existing, already-documented component
- No new architectural decisions are required
- Simple bug fix or isolated implementation change

**Execution**:
1. **MANDATORY**: Log any user input during this phase in audit.md
2. Load all steps from `inception/high-level-design.md`
3. Load reverse engineering artifacts (if brownfield), requirements, and user stories
4. Execute at appropriate depth (minimal/standard/comprehensive)
5. **Wait for Explicit Approval**: Present detailed completion message (see high-level-design.md for message format) - DO NOT PROCEED until user confirms
6. **MANDATORY**: Log user's response in audit.md with complete raw input

**Note**: High-Level Design produces the system context diagram, major component/service map, technology direction, and integration boundaries. Application Design and Units Generation then refine this into detailed component interfaces and units of work.
...
- Per-Unit Loop (executes for each unit):
  - Low-Level Design (CONDITIONAL, per-unit)
...
**Purpose**: Unit-level detailed design blueprint — module/class structure, interface contracts, and data/sequence flow — produced BEFORE business logic, NFR, and infrastructure design are worked out for the unit.

**Execute IF**:
- Unit has non-trivial internal structure (multiple modules/classes/interfaces)
- API contracts or interfaces need to be defined before implementation
- Sequence of interactions between internal parts is not obvious from High-Level/Application Design alone

**Skip IF**:
- Unit is a simple, single-module change
- Internal structure is trivial or fully implied by Functional Design

**Execution**:
1. **MANDATORY**: Log any user input during this stage in audit.md
2. Load all steps from `construction/low-level-design.md`
3. Execute low-level design for this unit
4. **MANDATORY**: Present standardized 2-option completion message as defined in low-level-design.md - DO NOT use emergent 3-option behavior
5. **Wait for Explicit Approval**: User must choose between "Request Changes" or "Continue to Next Stage" - DO NOT PROCEED until user confirms
6. **MANDATORY**: Log user's response in audit.md with complete raw input

**Note**: Low-Level Design produces the module/class structure, interface/API contracts, and sequence diagrams for the unit. Functional Design, NFR Design, and Infrastructure Design then fill in business logic, NFR patterns, and infrastructure mapping within that structure.
...
3. Generate comprehensive build and test instructions:
   ...
   - CI/CD integration instructions, including SonarQube static analysis and OWASP (Dependency-Check, and ZAP if a web/API service is exposed)
4. Create instruction files in build-and-test/ subdirectory: build-instructions.md, unit-test-instructions.md, integration-test-instructions.md, performance-test-instructions.md, ci-cd-integration-instructions.md, build-and-test-summary.md
...
**Purpose**: Deployment strategy, observability, resilience, and production-readiness

**Focus**: How to DEPLOY, RUN, and OPERATE it safely at the scale the NFRs promised
...
3. Load prior context: NFR requirements/design, infrastructure design, CI/CD integration instructions
4. Execute operations setup: deployment strategy, rollback plan, autoscaling verification against NFR targets, observability (metrics/tracing/logging/SLOs), incident response, disaster recovery (RPO/RTO), capacity planning
5. **Wait for Explicit Approval**: Present detailed completion message (see operations.md for message format) - DO NOT PROCEED until user confirms
6. **MANDATORY**: Log user's response in audit.md with complete raw input

**Note**: Operations does not re-decide architecture or infrastructure component choices — it operationalizes what HLD, NFR Design, and Infrastructure Design already decided, and cross-checks that the operational configuration (e.g., autoscaling limits) actually satisfies the NFR targets promised earlier.
